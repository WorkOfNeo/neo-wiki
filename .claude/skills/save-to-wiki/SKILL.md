...
The wiki's tag space is **a living folksonomy, not a closed schema**. Postgres stores tags as free-form `text[]`. Vocabulary is whatever has been used so far — the wiki documents itself by being used. This skill defines the *namespace meanings* (slow-changing principles); specific *values* emerge from data and evolve through use.

#### Step 1: Discover the live vocabulary

**Always call `wiki_tags` (optionally filtered by namespace) before proposing tags for a new entry.** This returns every distinct tag in current use with its usage count, sorted by frequency. Reuse existing values when they fit; introduce new values when they don't.

Example: composing an entry about a Bedrock rate limit gotcha you just hit:

```
// Discover what gotcha values are in play
wiki_tags({ namespace: "gotcha" })
// → ["gotcha:auth" (14), "gotcha:bedrock-region" (3), "gotcha:cors" (8), ...]
```

If `gotcha:rate-limit` doesn't show up but you hit one, just use it in your `wiki_write` call — it'll appear in `wiki_tags` next time. No skill edit required, no migration. The vocabulary IS the data.

#### Step 2: Pick namespaces by meaning

Each namespace describes a kind of relationship the entry has to your work. Definitions, not enumerations:

- **`client:<slug>`** — work done *for* a paying client. Value is the client's slug.
- **`product:<slug>`** — a specific Neo product or app the entry is about. Pick the most specific bucket; reserve a generic catch-all only for work that genuinely doesn't fit any product.
- **`stack:<slug>`** — a technology, framework, library, service, or platform meaningfully involved. Be liberal — these are the most useful filter dimension. Each distinct technology is its own value.
- **`pattern:<slug>`** — a **reusable solution shape** that would transfer to another project. Distinguish meaningfully — if "RAG" and "hybrid search" are different in your head, they should be different tags.
- **`gotcha:<slug>`** — a **specific non-obvious pitfall** worth flagging on its own. Describe the *trap*, not the technology. The bar: would future-you, months later, want to filter by *this kind of trap*? If yes, it deserves its own value.
- **`scope:<slug>`** — orthogonal axis: who the work is *for*. Use it when the entry's other tags don't already encode this distinction (e.g. an internal admin script for a Clerkr feature warrants `product:clerkr + scope:internal`).
- **`lang:<slug>`** — only when content is language-specific.

If you genuinely need a new namespace (the existing ones don't capture what the entry is about), introduce one — `meta:<slug>`, `audience:<slug>`, whatever — but pause first and check whether one of the above would do.

#### Step 3: Tag choice principles

1. **Anchor: at least one of `client:*` or `product:*`** — every entry needs a "what is this about" tag.
2. **Texture: 2–4 stack/pattern/gotcha tags** — entries with only an anchor tag are hard to retrieve later. Entries with `product:clerkr + stack:bedrock + gotcha:rate-limit + pattern:agentic-execution` are findable by every dimension that matters.
3. **Prefer specific over generic.** `gotcha:bedrock-region` beats `gotcha:auth` when both apply. Specificity makes the cluster labels meaningful and the search results sharper.
4. **Don't tag what isn't in the body.** A tag should map to something actually discussed. If `gotcha:cors` doesn't appear in the entry text, drop it.
5. **When introducing a new value**, slug-case it (lowercase, hyphens). The act of saving makes it part of the vocabulary; future sessions will see it via `wiki_tags`.

The skill is the constitution. The data is the dictionary. Don't conflate them.
